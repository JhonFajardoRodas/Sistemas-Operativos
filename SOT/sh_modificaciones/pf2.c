...
// TEcnologías
// Práctica Final: sh.c
// gcc -g -o pf2 -Wall -Wshadow pf2.c 
	...
	for (i = 0; i < n_commands; i++){
		...
		switch(array_pid[i]){
			...
			case 0:
				...
				p = execv(path_command, array_commands[i].arguments);
				if(p < 0){
...
int x;
	...
	for(;;){
		...
		cdm = tokenize(buffer2, ";", aux2);
		if(cdm > 1){
			for (x = 0; x < cdm; x++){

				strcpy(buffer, aux2[x]);
			

		
			a = tokenize(buffer, "#", aux);
			if(a > 1){
				strcpy(buffer, aux[0]);
				fich = aux[1];
				tokenize(fich, " \n", aux);
				fich = aux[0];
				//fd1 = creat(fich,0660);
				fd1 = open(fich , O_WRONLY | O_TRUNC | O_CREAT , 0660);
				if (fd1 < 0){
					err(1, "open file");
				}
				dup2(fd1, 2);
