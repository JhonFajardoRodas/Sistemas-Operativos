/* Ejercicio: 5
Jhon Fajardo Rodas   Tecnologias
*/

#include <stdlib.h>
#include <stdio.h>
#include <string.h>
#include <sys/types.h>
#include <unistd.h>
#include <err.h>
#include <sys/stat.h>
#include <fcntl.h>
#include <ctype.h>
#include <dirent.h>
#include <sys/wait.h>

struct dirent *reg_dir;
struct stat s;

char *aux[2048];


int
ok_txt(char *path)
{
	int ok = 0;

	char *key;
	key = strstr(path, ".txt");
	if ((key != NULL) && strlen(key) == strlen(".txt")){
		ok = 1;
	}



	return ok;
}


int
searcher(char *path, int n_bytes)
{

	DIR *d;
	int ok;
	int n=0;
	char path_new[2048];


	ok = stat(path, &s); // busco el status del path
	if (ok < 0){
		exit(EXIT_FAILURE);
	}
	if((s.st_mode & S_IFMT) == S_IFDIR)
	 { // compruebo si es un directorio
		d = opendir(path);
		if(d == NULL)
			exit(EXIT_FAILURE);
		for(;;)
		{
			reg_dir=readdir(d);
			if (reg_dir == NULL)
				break;
			if (reg_dir->d_name[0] == '.')
			{ // paso de los directorios raiz
				continue;
			}else{
				memset(path_new, 0, 2048);
				strcat(path_new, path);
				strcat(path_new, "/");
				strcat(path_new, reg_dir->d_name);

				ok=stat(path_new, &s); // compruebo el path construido
				if(ok<0)
					exit(EXIT_FAILURE);
				if((s.st_mode & S_IFMT) == S_IFREG)
				{ // si es un fichero compruebo si es un txt
					if(ok_txt(path_new))
					{ // aqui miro para imprimir los bytes
						aux[n] = strdup(path_new);
						n++;
					}
				}
			}
		}
	}

	return n;
}


void
creat_out(char *path_new, int n_bytes)
{
	int fd, nr, fd2;
	char buffer[1024];
	char path_2[2048];

	fd = open(path_new, O_RDONLY);
	if (fd < 0)
	{
		fprintf(stderr, "%s %s\n", "Fallo al abrir el fichero:   ", path_new);
	}

	if (n_bytes != 0)
		lseek(fd, -n_bytes, SEEK_END);
	nr = read(fd, buffer, sizeof buffer);
	if(nr < 0)
	{
		fprintf(stderr, "%s %s\n", "Fallo al leer el fichero:   ", path_new);
	}
	memset(path_2, 0, 2048);
	strcat(path_2, path_new);
	strcat(path_2, ".out");


	fd2 = creat(path_2, 0660);
	if (fd < 0)
	{
		err(1, "creat file");
	}

	write(fd2, buffer, nr);
	printf("\n");
	close(fd);
	close(fd2);

}

char*
ok_command(char *command)
{

	char dir_1[] = "/bin/";
	char dir_2[] = "/usr/bin/";
	char dir_3[] = "/usr/local/bin/";

	if (access(cat_command(command, dir_1), X_OK)==0){
		return cat_command(command, dir_1);
	}else if(access(cat_command(command, dir_2), X_OK)==0){
		return cat_command(command, dir_2);
	}else if (access(cat_command(command, dir_3), X_OK)==0){
		return cat_command(command, dir_3);
	}else {
		return NULL;
	}

}


int
main (int argc, char *argv[])
{

	char *cwd;
	char path[1024];
	int n_bytes = 0;
	int nFiles, pid;

	if (argc > 1)
		n_bytes = atoi(argv[1]);
 	cwd = getcwd(path, 64);
	nFiles = searcher(cwd, n_bytes);
	if(nFiles > 20)
	{
		fprintf(stderr, "%s\n", "Error: number of files exceeded");
		exit(EXIT_FAILURE);

	}


	int i;
	printf("%d\n", nFiles);
	for(i = 0; i < nFiles; i++)
	{

		pid = fork();
		switch(pid)
		{
			case -1:
				err(1, "fork failed");
				break;
			case 0:
				creat_out(aux[i], n_bytes);

				exit(EXIT_SUCCESS);
			default:
				wait(NULL);

		}

	}

	exit(EXIT_SUCCESS);
}
