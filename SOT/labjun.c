// Jhon Fajardo Rodas
// Tecnologías
// Práctica Final: shell.c
// gcc -g -o shell -Wall -Wshadow shell.c
// ME SALE UNA VIOLACION DE SEGMENTO Y BORRE LOS CAMBIOS
...
change_dir(void)
{
	...
	if(strcmp(array_commands[0].arguments[0], "cd") == 0){
		...
		}else if(strcmp(array_commands[0].arguments[1], "-") == 0){
			printf("----------%s\n", last_path);
			chg = chdir(last_path);
			last_path = getcwd(ruta, 1024);
...
*/
	...
	for(;;)
	{
		...
		// Modificacion del prompt

		/*
		getcwd(ruta, 1024);
		personal = getenv("HOME");
		e = strstr(ruta, personal);
		if(e != NULL){
			e = e + strlen(personal);
			strcpy(ruta2, "~");
			strcat(ruta2, e);
		...
		*/
		...
		/* Modificacion de salidad de error
		n_f = tokenize(buffer, ";", buffer_a);
		if (n_f > 1){
			f_err = buffer_a[1];
			tokenize(f_err, " \n", buffer_a2);
			f_err = buffer_a2[0];
			printf("[%s]\n", f_err);
			fd_e = creat(f_err,0660);
			if (fd_e < 0){
				err(1, "open file");
			}
			dup2(fd_e, 2);
			close(fd_e);	
			strcpy(buffer, buffer_a[0]);
		}
		*/
		...
		if (no_ve == 0){
			/* Modificacion del myuid
			if(strcmp(array_commands[0].arguments[0], "myuid") == 0){
				myuid = 1;
